    ...
    const getUser = async () => {
      ...
      if (isSuccess) {
        ...
        if (!user.profilePicture.image_url) {
          ...
          await setIsEmptyImageURL(true)
        } else {
          await setIsEmptyImageURL(false)
        }
          ...
          <Menu.Button className="relative flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
            ...
            {user !== null ? (
              <img
                ...
                src={isEmptyImageURL ? imagePlaceHolder : (user.user.profilePicture ? user.user.profilePicture.image_url : imagePlaceHolder)}
                alt=""
              />
            ) : (
              <img
